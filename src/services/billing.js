...
async function chargeOrStop(deployment) {
  ...
  const owner = await db.getUserById(deployment.user_id);
  const cost = owner && owner.plan === 'premium' ? s.premiumRenewalCostCoins : s.renewalCostCoins;
  const charged = await db.deductCoinsIfSufficient(deployment.user_id, cost);
  ...
  await db.updateDeploymentStatus(deployment.id, {
    status: 'stopped',
    failureMessage: `Paused — not enough JC for the ${cost} JC / ${s.renewalPeriodHours}h renewal. Top up and restart it from My Bots.`,
  });
}

async function runRenewalSweep() {
  const due = await db.getDeploymentsDueForRenewal(settings.get().renewalPeriodHours);
