...
const DEFAULTS = {
  ...
  // Brevo transactional email HTTP API — primary mail path. Unlike SMTP,
  // this goes out over plain HTTPS (port 443), which free-tier hosts
  // like Render allow (they block outbound SMTP ports 25/465/587).
  // Get a key from Brevo → Settings → SMTP & API → API Keys.
  // Leave blank to fall back to SMTP above (only works on hosts that
  // don't block SMTP ports), then to console-logging in dev.
  BREVO_API_KEY: '',
  BREVO_SENDER_EMAIL: 'mxdevzw@gmail.com',
  BREVO_SENDER_NAME: 'Jexploit platform',
  ...
  REFERRAL_BONUS_COINS: 5,           // paid to BOTH the referrer and the new user
  DEPLOY_COST_COINS: 10,             // flat cost to deploy any bot (also mirrored per-bot in data/bots.json)
  RENEWAL_COST_COINS: 10,            // charged every RENEWAL_PERIOD_HOURS to keep a deployed bot running
  ...
  DAILY_CLAIM_COINS: 5,              // free coins from the dashboard "Daily coins" button
  DAILY_CLAIM_PERIOD_HOURS: 24,
  PREMIUM_RENEWAL_COST_COINS: 5,     // discounted renewal rate for plan = 'premium'
  PREMIUM_BONUS_SLOTS: 10,           // extra bot slots on top of MAX_BOTS_PER_USER for premium
  ...
  // ── MarzPay (https://wallet.wearemarz.com) — primary top-up method.
  // Get API key + secret from your MarzPay dashboard → API Keys.
  // Leave blank to fall back to the manual PAYMENT_METHOD_LABEL flow above.
  MARZPAY_API_KEY: 'marz_UFsy6EVzcRmqkDWC',
  MARZPAY_API_SECRET: 'ub18BRW8PECvgzinS5CSAncqgLui6XpF',
  MARZPAY_WEBHOOK_SECRET: '', // optional — only if you enable webhook signing
  MARZPAY_COUNTRY: 'UG',
...
const config = {
  ...
  brevo: {
    apiKey: pick('BREVO_API_KEY'),
    senderEmail: pick('BREVO_SENDER_EMAIL'),
    senderName: pick('BREVO_SENDER_NAME', 'Jexploit platform'),
  },
  ...
  coins: {
    ...
    renewalCostCoins: num(pick('RENEWAL_COST_COINS'), 10),
    ...
    dailyClaimCoins: num(pick('DAILY_CLAIM_COINS'), 5),
    dailyClaimPeriodHours: num(pick('DAILY_CLAIM_PERIOD_HOURS'), 24),
    premiumRenewalCostCoins: num(pick('PREMIUM_RENEWAL_COST_COINS'), 5),
    premiumBonusSlots: num(pick('PREMIUM_BONUS_SLOTS'), 10),
  },
  ...
  // MarzPay automatic coin top-ups — primary payment method. Falls
  // back to the manual flow above (payments.methodLabel/number) when
  // not configured.
  marzpay: {
    apiKey: pick('MARZPAY_API_KEY'),
    apiSecret: pick('MARZPAY_API_SECRET'),
    webhookSecret: pick('MARZPAY_WEBHOOK_SECRET'),
    country: pick('MARZPAY_COUNTRY', 'UG'),
  },

  // Coin packages sold via MarzPay (amounts in UGX). Adjust prices to
  // match your preferred rate.
  coinPackages: [
    { coins: 10,  amountUgx: 5000,  label: '10 JC' },
    { coins: 20,  amountUgx: 10000, label: '20 JC', popular: true },
    { coins: 30,  amountUgx: 14000, label: '30 JC' },
    { coins: 60,  amountUgx: 25000, label: '60 JC' },
    { coins: 100, amountUgx: 40000, label: '100 JC' },
  ],
