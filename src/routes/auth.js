...
router.post('/verify', authLimiter, safe(
  ...
  async (req, res) => {
    ...
    flash(req, 'success', `Email verified — you've been credited ${settings.get().starterCoins} JC to get started (good for ${settings.get().starterCoinsExpiryDays} days).`);
...
router.get('/login', (req, res) => {
  if (req.session.userId) return res.redirect('/dashboard');
  if (req.query.banned) {
    flash(req, 'error', 'This account has been suspended. Contact support if you think this is a mistake.');
  }
